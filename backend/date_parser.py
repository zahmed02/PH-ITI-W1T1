...
def parse_time_expression(expr: str) -> tuple[int, int] | None:
    ...
    if match:
        ...
        # For simplicity, we'll treat the minute as 0 and slot from hour to hour+1
        # But if minute is specified, we could adjust, but keep simple.
        # We'll set start hour to hour, end hour to hour+1.
        end_hour = hour + 1
        if end_hour > 24:
            end_hour = 24
